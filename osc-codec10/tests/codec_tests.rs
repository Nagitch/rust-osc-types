...
use osc_codec10::{encode_message, decode_message, encode_bundle, decode_bundle};

#[test]
fn roundtrip_message_basic() {
    let msg = Message::new("/synth/volume",
        vec![OscType::Float(0.5), OscType::String("foo"), OscType::Int(42)]);
...
fn roundtrip_blob_padding() {
    ...
    let (m2, used) = decode_message(&bytes).unwrap();
    assert_eq!(used, bytes.len());
    match &m2.args[0] { OscType::Blob(b) => assert_eq!(*b, &blob), _ => panic!() }
}

#[test]
fn roundtrip_bundle_basic() {
    let msg = Message::new("/ping", vec![OscType::Int(7)]);
    let b = Bundle::new(1u64, vec![msg]);
